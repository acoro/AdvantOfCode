...
print("Advent Of Code 2023 - Exercice 4_2")
...
Liste_Instance=[0]*len(Liste_Resultat)

for i in range(len(Liste_Resultat)):
   if (Liste_Resultat[i])==0:
      Liste_Instance[i]=Liste_Instance[i]+1
   else :
      Liste_Instance[i]=Liste_Instance[i]+1
   for j in range(Liste_Resultat[i]) :
      Liste_Instance[i+j+1]=int(Liste_Instance[i+j+1])+Liste_Instance[i]
   
for i in Liste_Instance:
   Somme_Instance=Somme_Instance+i
   
print("_____________________________")
print("Le nombre d'instance est :", Somme_Instance)
